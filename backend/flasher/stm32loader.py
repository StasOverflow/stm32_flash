...
class Stm32Bootloader:
    ...
    def __init__(self, swap_rts_dtr=False, reset_active_high=False, boot0_active_high=False, callback=None,
                 logger=None):
        self.serial = None
    ...
    def log(self, data, error=None):
        if callable(self.logger):
            if data is not None:
                self.logger(data, error)
    ...
    def open(self, serial_port, baud_rate=115200, parity=serial.PARITY_EVEN):
        try:
            ...
            line = 'could not open port ' + serial_port
            self.log(line, 404)
    ...
    def get(self):
        ...
        self.log(debug(10, "    Bootloader version: " + hex(version)), None)
    ...
    def get_version(self):
        ...
        self.log(debug(10, "*** GetVersion command"), None)
        version = bytearray(self.serial.read())[0]
        self.serial.read(2)
    ...
    def get_id(self):
        ...
        self.log(debug(10, "*** GetID command"), None)
        length = bytearray(self.serial.read())[0]
    ...
    def read_memory(self, address, length):
        ...
        self.log(debug(10, "*** ReadMemory command"), None)
    ...
    def write_memory(self, address, data):
        ...
        self.log(debug(10, "*** Write memory command"), None)
        self.serial.write(self._encode_address(address))
        ...
        self.log(debug(10, "    Write memory done"), 200)
    ...
    def erase_memory(self, sectors=None):
        ...
        self.log(debug(10, "    Erase memory done"), 200)
    ...
    def read_memory_data(self, address, length):
        ...
        if callable(self._callback):
            do_callback = True
        while length > 256:
            percentage = int((1 - length / len_curr) * 100)
            print(percentage)
            # if callable(self._callback):
            ...
            if do_callback:
                pass
                self._callback(percentage)
            self.log(debug(5, "Read %(len)d bytes at 0x%(address)X" % {'address': address, 'len': 256}), None)
    ...
    def write_memory_data(self, address, data):
        """
            length is a total length of data
        """
        ...
        percentage = 0
        while length > 256:
            percentage = int((1 - length / length_total) * 100)
            print(percentage)
            if callable(self._callback):
                self._callback(percentage)
            self.log(debug(5, "Write %(len)d bytes at 0x%(address)X" % {'address': address, 'len': 256}), None)
...
class Stm32Loader:
    ...
    def parse_arguments(self, arguments, callback=None):
        # self.log("ADDDDDDDDDDDDDDDD")
        print(arguments)
    ...
    def connect(self):
        ...
        self.log(debug(10, "Open port %(port)s, baud %(baud)d" % {
        ...
        try:
            ...
            self.log("Can't init device", 404)
    ...
    def perform_commands(self):
        ...
        if self.configuration['unprotect']:
            try:
                ...
                # self.log(debug(0, "Erase failed -- probably due to readout protection"))
                self.log(debug(0, "Erase failed"), 404)
                self.log(debug(0, "Quit"), 404)
                self.bootloader.reset_from_flash()
                sys.exit(1)
...
"""
    ...
    def read_device_details(self):
        ...
        self.log(debug(0, "Bootloader version %X" % boot_version), None)
        device_id = self.bootloader.get_id()
        self.log(debug(0, "Chip id: 0x%x (%s)" % (device_id, CHIP_IDS.get(device_id, "Unknown"))), None)
