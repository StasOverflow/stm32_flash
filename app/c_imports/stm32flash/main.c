...
int main(int argc, char** argv) {
	...
	{
		fprintf(stderr, "Successfully opened port %s\n", port_opts.device);
		fprintf(stderr, "With baudrate %d\t\n", port_opts.baudRate);
		fprintf(stderr, "With serial mode %s\t\n", port_opts.serial_mode);
		fprintf(stderr, "With bus addres %d\t\n", port_opts.bus_addr);
		fprintf(stderr, "with rx_max %d of %d\t\n", port_opts.rx_frame_max, STM32_MAX_RX_FRAME);
		fprintf(stderr, "with tx_max %d of %d\t\n", port_opts.tx_frame_max, STM32_MAX_TX_FRAME);
	}
	...
	if (start_addr || readwrite_len) {
		...
		fprintf(stderr, "im in start || readwrite \n");
		...
		fprintf(stderr, "im in !spage && !npage \n");
		start = stm->dev->fl_start;
		end = stm->dev->fl_end;
		first_page = 0;
		num_pages = STM32_MASS_ERASE;
	} else {
		fprintf(stderr, "im in else \n");
		first_page = spage;
